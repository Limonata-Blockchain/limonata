...
// transparent path does NOT use it — there the threshold is t = floor(2S/3)+1 of the
// evaluation-point budget S, and admission fail-closes any active apportionment where
// <=2/3 stake can still reconstruct.
...
func (k Keeper) dealingWeightUpperBound(ctx context.Context, round types.DkgRound) int {
	...
	k.IterateDealings(ctx, round.Epoch, func(d types.Dealing) {
		if seen[d.DealerIndex] {
			return
		}
		seen[d.DealerIndex] = true
		total += weightOf[d.DealerIndex]
	})
	return total
}
...
func (k Keeper) finalizeRound(ctx sdk.Context, round types.DkgRound) {
	...
	// above the reconstruction point threshold participated, which for the unweighted path
	// reduces EXACTLY to the original |QUAL| >= t check.
