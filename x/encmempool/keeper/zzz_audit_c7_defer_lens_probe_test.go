...
func TestAudit_C7_NeverHeals_DropsStrandedAtGraceEnd_H2Safe(t *testing.T) {
	...
	if streak := c.k.GetDecryptStrandStreak(c.ctx, e.Epoch); streak != 1 {
		t.Fatalf("MED-2: a stranded maturity must bump the epoch's decrypt-health streak to 1, got %d", streak)
	}
