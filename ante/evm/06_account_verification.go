...
//
// When sponsored is true the protocol gas pool pays the fee, so the affordability check
// only requires the sender to cover the transferred value (see CheckSenderBalance).
func VerifyAccountBalance(
	...
	if err := keeper.CheckSenderBalance(sdkmath.NewIntFromBigInt(account.Balance.ToBig()), ethTx, sponsored); err != nil {
